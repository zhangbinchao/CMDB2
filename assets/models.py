...
class Asset(models.Model):
    ...
    business_unit = models.ForeignKey('BusinessUnit','on_delete',null=True, blank=True, verbose_name='所属业务线')
    status = models.SmallIntegerField(choices=asset_status, default=0, verbose_name='设备状态')

    # manufacturer = models.ForeignKey('Manufacturer','on_delete', null=True, blank=True, verbose_name='制造商')
    ...
    admin = models.ForeignKey(User, 'on_delete',null=True, blank=True, verbose_name='资产管理员', related_name='admin')
    # idc = models.ForeignKey('IDC','on_delete', null=True, blank=True, verbose_name='所在机房')
    idc = models.CharField(max_length=128,  null=True,verbose_name='所在机房')
    contract = models.ForeignKey('Contract','on_delete', null=True, blank=True, verbose_name='合同')
    ...
    approved_by = models.ForeignKey(User,'on_delete', null=True, blank=True, verbose_name='批准人', related_name='approved_by')
...
class Server(models.Model):
    ...
    asset = models.OneToOneField('Asset','on_delete')  # 非常关键的一对一关联！
...
class CPU(models.Model):
    ...
    asset = models.OneToOneField('Asset','on_delete')  # 设备上的cpu肯定都是一样的，所以不需要建立多个cpu数据，一条就可以，因此使用一对一。
...
class RAM(models.Model):
    ...
    asset = models.ForeignKey('Asset','on_delete')  # 只能通过外键关联Asset。否则不能同时关联服务器、网络设备等等。
...
class NIC(models.Model):
    ...
    asset = models.ForeignKey('Asset','on_delete')  # 注意要用外键
...
class BusinessUnit(models.Model):
    ...
    parent_unit = models.ForeignKey('self','on_delete', blank=True, null=True, related_name='parent_level')
